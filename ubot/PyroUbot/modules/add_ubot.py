...
async def _(client, message):
    ...
    pantek = "https://files.catbox.moe/pgaypk.jpg"
...
""",
    ...
    if user_id not in premium_users and user_id not in ultra_premium_users:
        buttons = [
            [InlineKeyboardButton("🛒 ᴘɪʟɪʜ ʀᴏʟᴇ", callback_data=f"belirole {user_id}")],
            [InlineKeyboardButton("ᴋᴇᴍʙᴀʟɪ", callback_data=f"home {user_id}")],
        ]
        return await smart_edit(
            callback_query,
...
ɪᴅ : {new_client.me.id}
ᴘʀᴇꜰɪxᴇꜱ : {' '.join(SH)}
ʜᴀʀᴀᴘ ᴊᴏɪɴ : https://t.me/+dMN6eaC7BEU3OWU1 ʜᴀʀᴀᴘ ᴊᴏɪɴ ɢʀᴜᴘ ᴅɪᴀᴛᴀs ᴅᴀɴ ᴊᴀɴɢᴀɴ ᴏᴜᴛ ᴀɢᴀʀ sᴀғᴇᴛʏ
ᴊɪᴋᴀ ʙᴏᴛ ᴛɪᴅᴀᴋ ʀᴇꜱᴘᴏɴ, ᴋᴇᴛɪᴋ /restart</b></blockquote>
    ...
    try:
        await new_client.join_chat(MAIN_CHANNEL)
        await new_client.join_chat(NOTIFY_CHANNEL)
    except UserAlreadyParticipant:
        pass

    await notify_ubot_aktif(new_client.me)
...
async def notify_ubot_aktif(account):
    try:
        notified = await get_list_from_vars(bot.me.id, "NOTIFIED_UBOT")
        if account.id in notified:
            return
        await add_to_vars(bot.me.id, "NOTIFIED_UBOT", account.id)
        waktu = datetime.now(timezone("Asia/Jakarta")).strftime("%d-%m-%Y %H:%M:%S")
        await bot.send_photo(
            NOTIFY_CHANNEL,
            photo=NOTIFY_THUMBNAIL,
            caption=MSG.NOTIFY_UBOT(account, waktu),
        )
    except Exception as error:
        print(f"[INFO] - gagal kirim notif ubot: {error}")
